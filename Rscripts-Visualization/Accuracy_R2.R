...
tcp_metrics <- data.frame(
  TrafficType = c("TCP1","TCP2","TCP3","TCP4"),
  TNR   = c(ct.p.tcp.specificity, st.p.tcp.specificity,c.p.tcp.specificity, s.p.tcp.specificity),
  TPR      = c(ct.p.tcp.recall, st.p.tcp.recall, c.p.tcp.recall, s.p.tcp.recall),
  F1          = c(ct.p.tcp.f1, st.p.tcp.f1,c.p.tcp.f1, s.p.tcp.f1)
)

udp_metrics <- data.frame(
  TrafficType = c("UDP1","UDP2","UDP3","UDP4"),
  TNR   = c(ct.p.udp.specificity, st.p.udp.specificity, c.p.udp.specificity, s.p.udp.specificity),
  TPR      = c(ct.p.udp.recall,st.p.udp.recall, c.p.udp.recall,s.p.udp.recall),
  F1          = c(ct.p.udp.f1,st.p.udp.f1,c.p.udp.f1,s.p.udp.f1)
)
...
bp_tcp <- barplot(
  ...
  beside     = TRUE,
  names.arg  = c(expression(D[w]), expression(D[w]^{T}),
                 expression(D[wo]), expression(D[wo]^{T})),
  col        = fill_colors,        # solid fill colors
  border     = fill_colors,        # solid borders
  ylim       = c(0, 1),
  ylab       = "Score",
...
bp_udp <- barplot(
  ...
  beside     = TRUE,
  names.arg  = c(expression(D[w]), expression(D[w]^{U}),
                 expression(D[wo]), expression(D[wo]^{U})),
  col        = fill_colors,        # solid fill colors
  border     = fill_colors,        # solid borders
  ylim       = c(0, 1),
  ylab       = "Score",
...
tcp_metrics <- data.frame(
  TrafficType = c("TCP1","TCP2","TCP3","TCP4"),
  TNR   = c(ct.f.tcp.specificity, st.f.tcp.specificity,c.f.tcp.specificity, s.f.tcp.specificity),
  TPR      = c(ct.f.tcp.recall, st.f.tcp.recall, c.f.tcp.recall, s.f.tcp.recall),
  F1          = c(ct.f.tcp.f1, st.f.tcp.f1,c.f.tcp.f1, s.f.tcp.f1)
)

udp_metrics <- data.frame(
  TrafficType = c("UDP1","UDP2","UDP3","UDP4"),
  TNR   = c(ct.f.udp.specificity, st.f.udp.specificity, c.f.udp.specificity, s.f.udp.specificity),
  TPR      = c(ct.f.udp.recall,st.f.udp.recall, c.f.udp.recall,s.f.udp.recall),
  F1          = c(ct.f.udp.f1,st.f.udp.f1,c.f.udp.f1,s.f.udp.f1)
)
