...
header ipv4_options_t {
	...
	bit<16> switch_delay; //pipeline delay of a packet
	bit<16> pkts; //packets in a flow
	...
	*******************************************************************/
	...
	//XX For rate comparisons 'th' is multiplied by time delta as division is not allowed
		...
		if (f == 1) {
		     	...
		     	feature = (bit<64>)meta.min_iat; // min. inter packet duration
			th = th*(bit<64>)UNITS;
		}
		else if (f == 6) {
	...
	//************ CHECK FEATURES UDP ********************************************
        ...
        //XX For rate comparisons 'th' is multiplied by time delta as division is not allowed
			...
			feature = (bit<64>)meta.total_dur; // flow duration
			th = th*(bit<64>)UNITS;
		}
		else if (f ==6) {
		     	feature = (bit<64>)meta.min_iat; // min. inter packet duration
