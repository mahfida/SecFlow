...
header ipv4_options_t {
	...
	bit<16> switch_delay; //time spent by a packet in p4 pipeline
	...
	*******************************************************************/
	...
	//XX For rate comparisons 'th' is multiplied by time delta as division is not allowed
		...
		bit<64> feature = 0;
        	bit<64> th = 0;
		bit<16> f = f_inout + 1;
		bit<64> thdur = threshold*(bit<64>)meta.total_dur;
		bit<64> thunit =threshold*(bit<64>)UNITS;	
	
		if (f == 1) {
                        feature = (bit<64>)(meta.sbytes + meta.dbytes); // Flowbytes
                }
		else if (f == 2) {
                        feature = (bit<64>)meta.srate; //srate
                    	th = thdur;
                }
                else if (f == 3) {
                        feature = (bit<64>)meta.drate; //drate
                        th = thdur;
                }
		else if (f == 4) {
                        ...
                        feature = (bit<64>)meta.max_iat; // max. inter-packet duration
			th = thunit;
                }
		else if (f == 7) {
