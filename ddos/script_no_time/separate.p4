...
header ipv4_options_t {
	...
	bit<16> switch_delay; //time spent by packet in p4 pipeline
	bit<16> pkts; //pkts sent in a flow
...
control MyIngress(inout headers hdr,
	...
	counter(1, CounterType.packets) number_attack_pkts;//Total number of packets detected as attack
    	counter(1, CounterType.packets) counter_flows;
     	counter(1, CounterType.packets) controller_packets;
	...
	*******************************************************************/
	...
	//***features********* CHECK FEATURES TCP ********************************************
    	action CheckFeatureTCP(bit<16> node_id, bit<16> f_inout, bit<64> threshold) {
	...
	//XX For rate comparisons 'th' is multiplied by time delta as division is not allowed
		...
		if (f == 1) {
                        ...
                        feature = (bit<64>)meta.spkts; //spkts
                }
                else if (f == 3) {
                        feature = (bit<64>)meta.dpkts; //dpkts
                }
		else if (f == 4) {
			...
			th = th*(bit<64>)(meta.pkts);
	    		...
	    		feature = (bit<64>)meta.state_CON;//reset CON
		}
		else if (f == 9){
			feature = (bit<64>)meta.state_RST;//RST
		}
		else if (f == 10) {
	...
	//************ CHECK FEATURES UDP ********************************************
        action CheckFeatureUDP(bit<16> node_id, bit<16> f_inout, bit<64> threshold) {
        //XX For dur and tcprtt are in microseconds! Thus multiplied by 1000000 if in seconds!
        //XX For rate comparisons 'th' is multiplied by time delta as division is not allowed
    	...
    	//************ SET CLASS TCP ********************************************
    	action SetClassTCP(bit<16> node_id, bit<16> class) {
		meta.class = class;
		meta.node_id = node_id; //just for debugging otherwise not needed
    	}
	//************ SET CLASS UDP ********************************************
        action SetClassUDP(bit<16> node_id, bit<16> class) {
                meta.class = class;
                meta.node_id = node_id; //just for debugging otherwise not needed
        }
	...
	action sdn(){
		...
		hdr.ipv4_option.switch_delay = (bit<16>)(standard_metadata.deq_timedelta);
		hdr.ipv4_option.pkts= (bit<16>)(meta.dpkts) + (bit<16>)(meta.spkts);
