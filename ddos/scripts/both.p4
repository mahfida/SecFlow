		...
		//TODO optimize : feature needs to be calculated only if needed
			if (hdr.ipv4.protocol == 6 || hdr.ipv4.protocol == 17) {//We treat only TCP or UDP packets	
				...
				if (meta.direction == 1) {
					...
					//if(hdr.ipv4.protocol == 6 && standard_metadata.packet_length == 60){
                                        //        meta.sbytes = meta.sbytes + standard_metadata.packet_length -6;
                                        //}else{
			...
			//************* DIRECTION 0 ************************************************
					...
					reg_dbytes.read(meta.dbytes, (bit<32>)meta.register_index);
					//if(hdr.ipv4.protocol == 6 && standard_metadata.packet_length == 60){
					//	meta.dbytes = meta.dbytes + standard_metadata.packet_length -6;
					//}else{
